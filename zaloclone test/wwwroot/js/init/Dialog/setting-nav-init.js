function initSettingModal() {
    ...
    const changePasswordButton = document.querySelector('.setting-btn:nth-child(2)'); // Lấy nút Đổi mật khẩu
    ...
    if (changePasswordButton && changePassDialog) {
        ...
        document.addEventListener('click', function (event) {
            if (changePassDialog.style.display === 'flex' && !changePassDialog.contains(event.target) && !changePasswordButton.contains(event.target)) {
